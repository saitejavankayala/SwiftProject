...
extension MenuViewController: SideMenuViewControllerDelegate {
    func selectedCell(_ row: Int) {
        ...
        case 2:
            // Logout
            self.setNavBarAppearance(tintColor: .white, barColor: UIColor .white)
...
extension UIViewController {
    // With this extension you can access the MenuViewController from the child view controllers.
    func revealViewController() -> MenuViewController? {
        var viewController: UIViewController? = self
        if viewController != nil && viewController is MenuViewController {
            return viewController! as? MenuViewController
        }
        while (!(viewController is MenuViewController) && viewController?.parent != nil) {
